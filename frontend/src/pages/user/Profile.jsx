...
const handleSave = async () => {
  try {
    ...
    // ✅ Kiểm tra lỗi từ Laravel (422)
    if (err.response?.status === 422) {
      const errors = err.response.data.errors;
      if (errors) {
        // Lặp qua các lỗi và hiển thị tất cả
        Object.values(errors).forEach((msgArray) => {
          toast.error(msgArray[0]); // lấy message đầu tiên trong mỗi trường
        });
      } else {
        toast.error(err.response.data.message || "❌ Dữ liệu không hợp lệ!");
      }
    } else {
      toast.error(err.response?.data?.message || "❌ Đã xảy ra lỗi máy chủ!");
    }
  }
};
