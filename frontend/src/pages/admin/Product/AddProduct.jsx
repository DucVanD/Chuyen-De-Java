...
import { FaArrowLeft, FaPlus, FaImage, FaList, FaTag, FaInfoCircle, FaDollarSign, FaBoxOpen } from "react-icons/fa";
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 animate-fadeIn pb-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
        <div>
          <h3 className="text-2xl font-bold text-slate-800">Thêm sản phẩm mới</h3>
          <p className="text-slate-500 text-sm mt-1">Khởi tạo thông tin, giá bán và hình ảnh cho sản phẩm mới</p>
        </div>
        <Link
          to="/admin/products"
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className={`w-full px-4 py-3 rounded-xl border transition-all outline-none ${errors.name ? "border-red-500 ring-4 ring-red-500/10" : "border-slate-200 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10"}`}
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Tóm tắt ngắn gọn các đặc điểm..."
                  required
                ></textarea>
              </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 space-y-5">
            ...
            <div className="space-y-4">
                ...
                <label className="flex flex-col items-center justify-center w-full h-48 border-2 border-dashed border-slate-200 rounded-2xl cursor-pointer hover:bg-slate-50 transition-all duration-200 overflow-hidden group">
                  {thumbPreview ? (
                    <div className="relative w-full h-full">
                      ...
                      <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center text-white text-sm font-bold">Thay đổi ảnh</div>
                    </div>
                  ) : (
                    <div className="flex flex-col items-center justify-center pt-5 pb-6 text-slate-400 group-hover:text-indigo-500">
                      <FaImage className="w-10 h-10 mb-3 transition-colors" />
              ...
              <div>
                <label className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Trạng thái mặc định</label>
                <select
                  name="status"
                  value={formData.status}
                  onChange={handleChange}
                  className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 transition-all outline-none bg-white font-semibold text-indigo-600"
                >
          ...
          <div className="bg-slate-900 p-6 rounded-2xl shadow-xl shadow-slate-200 space-y-4">
            ...
            >
              {loading ? (
                ...
                <>
                  <FaPlus className="mr-2" /> Tạo sản phẩm ngay
                </>
              )}
            </button>
          </div>
