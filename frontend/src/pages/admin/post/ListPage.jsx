...
const ListPage = () => {
    ...
    const fetchPages = async (page = 1) => {
        try {
            setLoading(true);
            const res = await apiPostAdmin.getPage(page - 1, 8, "PAGE");
    ...
    const handleDelete = async (id) => {
        if (!window.confirm("Bạn có chắc muốn xóa trang này không?")) return;
        try {
                                    ...
                                    pages.map((item) => (
                                        <tr key={item.id} className="hover:bg-gray-50 border-b border-gray-100">
                                                ...
                                                <div className="flex-shrink-0 h-14 w-20">
                                                    {item.image ? (
                                                        ...
                                                        <div className="h-14 w-20 bg-gray-50 border border-gray-200 rounded-md flex items-center justify-center">
                                                            <span className="text-gray-400 text-[10px] text-center px-1">Không có hình</span>
                                                        </div>
                                                    )}
                                                </div>
                                            </td>
                                            <td className="px-4 py-3 text-sm text-gray-800">
                                                ...
                                                <div className="text-gray-400 text-[11px] mt-0.5">Slug: {item.slug}</div>
                                            </td>
                                            ...
                                            <td className="px-4 py-3">
                                                {item.status === 1 ? (
                                                    <span className="px-2 py-0.5 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-700">Hiển thị</span>
                                                ) : (
                                                    <span className="px-2 py-0.5 inline-flex text-xs leading-5 font-semibold rounded-full bg-gray-100 text-gray-600">Ẩn</span>
                                                )}
                                            </td>
